        ...
        {phase === 'countdown' ? (
          ...
          >
            ...
            <div className="bg-surface border border-surfaceHighlight rounded-2xl p-4 mb-4">
              ...
              {/* Broadcast SMS to all contacts button */}
              {state.contacts.filter(c => c.phone).length > 0 && (
                <div className="mb-3 p-2.5 bg-sky-500/10 border border-sky-500/20 rounded-xl flex items-center justify-between">
                  <div className="flex items-center gap-2">
                    <MessageSquare className="w-4 h-4 text-sky-400" />
                    <span className="text-xs font-bold text-white">Direct SMS Broadcast</span>
                  </div>
                  <a
                    href={getSmsUrl(
                    ...
                    className="px-2.5 py-1 bg-sky-500 hover:bg-sky-400 text-black text-[11px] font-bold rounded-lg flex items-center gap-1 transition-all"
                  >
              ...
              <div className="space-y-3">
                {((state.activeAlert?.contactsNotified && state.activeAlert.contactsNotified.length > 0)
                  ...
                  const emergencyMsg = buildEmergencyText({
                    userName: state.userName,
                    alertType: state.activeAlert?.type || 'Emergency',
                    lat: location.lat,
                    lng: location.lng,
                  ...
                  return (
                    <div key={c.id || c.contactId} className="bg-background/60 border border-white/5 rounded-xl p-3">
                      ...
                      <div className="flex gap-2">
                        ...
                        {phone && (
                          <a
                            href={getWhatsAppUrl(phone, emergencyMsg)}
                            target="_blank"
                            rel="noopener noreferrer"
                        ...
                        {phone && (
                          <a
                            href={getSmsUrl(phone, emergencyMsg)}
