...
import { getNativeSmsUri, getWhatsAppDirectUri, buildNativeEmergencyText } from '../utils/nativeSms';
        ...
        {phase === 'countdown' ? (
          ...
          >
            ...
            <div className="bg-surface border border-surfaceHighlight rounded-2xl p-4 mb-4">
              ...
              {/* Free One-Tap Device Native SMS Broadcast to All Contacts */}
              {state.contacts.filter(c => c.phone).length > 0 && (
                <div className="mb-3.5 p-3 bg-sky-500/10 border border-sky-500/30 rounded-2xl">
                  <div className="flex items-center justify-between gap-2">
                    <div className="flex items-center gap-2">
                      ...
                      <div>
                        <p className="text-xs font-bold text-white">Free Native SMS Broadcast</p>
                        <p className="text-[10px] text-textMuted">Open phone's SMS app with all contacts</p>
                      </div>
                    </div>
                    <a
                      href={getNativeSmsUri(
                        ...
                        buildNativeEmergencyText({
                          ...
                          alertType: state.activeAlert?.type || 'Emergency',
                          lat: location.lat,
                          lng: location.lng,
                          alertId: alertId || undefined
                        })
                      )}
                      className="px-3 py-2 bg-sky-500 hover:bg-sky-400 text-black text-xs font-black rounded-xl flex items-center gap-1.5 shadow-lg shadow-sky-500/20 transition-all shrink-0"
                    >
              ...
              <div className="space-y-3">
                {((state.activeAlert?.contactsNotified && state.activeAlert.contactsNotified.length > 0)
                  ...
                  const emergencyText = buildNativeEmergencyText({
                    userName: state.userName,
                    alertType: state.activeAlert?.type || 'Emergency',
                    lat: location.lat,
                    lng: location.lng,
                  ...
                  return (
                    <div key={c.id || c.contactId} className="bg-background/60 border border-white/5 rounded-xl p-3">
                      ...
                      <div className="flex gap-2">
                        ...
                        {phone && (
                          <a
                            href={getWhatsAppDirectUri(phone, emergencyText)}
                            target="_blank"
                            rel="noopener noreferrer"
                        ...
                        {phone && (
                          <a
                            href={getNativeSmsUri(phone, emergencyText)}
                            className="flex-1 py-1.5 bg-sky-500/15 hover:bg-sky-500/25 border border-sky-500/30 text-sky-400 rounded-lg text-[10px] font-bold flex items-center justify-center gap-1 transition-all"
                          >
